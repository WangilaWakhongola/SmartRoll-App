...
export default function StudentProfile() {
  ...
  async function handleBiometricToggle(val: boolean) {
    ...
    Alert.alert(val ? 'Biometrics Enabled' : 'Biometrics Disabled', val ? 'You can now use fingerprint or Face ID to sign attendance.' : 'Biometric login has been turned off.')
  }

  const infoRows = [
    { key: 'Student ID',    val: user?.studentId ?? 'STU/2022/001' },
    { key: 'Email',         val: user?.email ?? 'student@campus.edu' },
    { key: 'Programme',     val: 'BSc. Computer Science' },
    { key: 'Academic Year', val: 'Year 2, Semester 1' },
  ]
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={styles.avatarCard}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.fullName}>{user?.fullName ?? 'Student'}</Text>
            <Text style={styles.roleText}>Student  SmartRoll</Text>
        ...
        <View style={styles.section}>
          {infoRows.map((row, i) => (
            <View key={row.key} style={[styles.row, i < infoRows.length - 1 && styles.rowBorder]}>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.row, styles.rowBorder]}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.rowValue}>{notifEnabled ? 'Push Alerts Enabled' : 'Push Alerts Disabled'}</Text>
            </View>
            <Switch
              value={notifEnabled}
              onValueChange={val => {
                setNotifEnabled(val)
                Alert.alert(val ? 'Notifications On' : 'Notifications Off', val ? 'You will receive attendance alerts.' : 'Push notifications have been turned off.')
              }}
